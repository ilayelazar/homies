                ...
                <div class="col-md-12 col-lg-12" panel>
                    ...
                    <div class="panel-heading" id="syllabus">
                        <div class="tabbable">
							...
							<span class="moduleHeader"> Admin panel </span>
        ...
        if ($conn->connect_error) {
          ...
          echo "<script>console.log('DB Connection succeded');</script>";
							...
							if ($result->num_rows > 0) {
								 echo "<h1>Waiting list: </h1><table class='user-manage cell-border row-border hover order-column homiesTables'>
                                    <br>
                                    <thead>
                                        <tr id='tableHeaders'>
                                            <th>First name </th>
								 ...
								 while($row = $result->fetch_assoc()) {
									 echo "<tr>
											...
											echo "</td>
                                            <td><a href='#' onclick=\"edituser('".$row['username']."','".$_SESSION['user']."',1);\"><img id='approveUser' src='img/correct-symbol.png' title='Deny user'></a></td>
											<td><a href='#' onclick=\"edituser('".$row['username']."','".$_SESSION['user']."',0);\"><img id='removeUser' src='img/remove-symbol.png' title='Approve user'></a></td>
								 ...
								 echo "</tbody></table>";
									 ...
									 echo "<table class='user-manage cell-border row-border hover order-column homiesTables'>
                                    <br>
                                    <thead>
                                        <tr id='tableHeaders'>
                                            <th>First name </th>
...
<!--------------GROUP USERS TABLE----------------------------- -->									
        ...
        if ($conn->connect_error) {
          ...
          echo "<script>console.log('DB Connection succeded');</script>";
							...
							if ($result->num_rows > 0) {
								 echo "<h1>My Family: </h1><table class='homiesTables'>
                                    <br>
                                    <thead>
                                        <tr id='tableHeaders'>
                                            <th>First name </th>
								 ...
								 while($row = $result->fetch_assoc()) {
									 echo "<tr>                                      						 
											...
											{
												echo "<input type='text'  value='" . $row['score']. "' name='currScore'></form>";
											}
                          ...
                          <td><button onclick=\"editUserScore('".$row['username']."');\" id='apply_".$row['username']."'>Apply</button</td>
                            ...
                            <script>
                             function editUserScore(user){ 
                                var newP = document.getElementById("apply_"+user).parentElement.parentElement.children[3].children[0].value;

                                var newPoints = document.getElementById("apply_"+user).parentElement.parentElement.children[4].children[0].value;
        ...
        if ($conn->connect_error) {
          ...
          echo "<script>console.log('DB Connection succeded');</script>";
							...
							if ($result->num_rows > 0) {
								 echo "<table class= 'homiesTables user-manage'>
                                   <thead>
                                        <tr>  
                                            <th>Present name</th>
                                            <th>Description</th>
								 ...
								 while($row = $result->fetch_assoc()) {
									 echo "<tr>
											...
											<td ><a class='links' href='" . $row['p_link']. "' target='_blank'>" . $row['p_link']. "</a></td>
